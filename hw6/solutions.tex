...
\item The data in Table \ref{tab:p3_data}, taken from Wakefield et al. (1994),
  ...
  \item Plot the data, along with the fitted curve.
    ...
    \end{figure}
    \begin{description}
    \item[Solution:] Plots of the data along with the fitted curve and 95\%
      confidence interval regions can be seen in Figures \ref{fig:p3_fitted} and
      \ref{fig:p3_fitted_log}. In the first plot, the untransformed data is
      plotted. In the second, a $\log$-scaling is applied to the drug
      concentration. Errors appear to increase with time.
    \end{description}
  ...
  \item The clearance $Cl = V \times k_e$ and elimination half-life
    ...
    \item[Solution:] The MLE is invariant to reparameterization, so computing
      ...
      \href{nbviewer.jupyter.org/github/ppham27/stat570/blob/master/hw6/cadralazine.ipynb}{\texttt{hw6/cadralazine.ipynb}}.
    \end{description}
  \end{enumerate}
\end{enumerate}
\end{document}
