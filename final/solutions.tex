...
\item
  ...
  \item A more complex likelihood for these data would assume that the $i$-th
    ...
    \item Using the previous part, write down the likelihood function
      ...
      \begin{description}
      \item[Solution:] We can substitute Equations \ref{eqn:p2_pmf_beta} and
        \ref{eqn:p2_survival_beta} into Equation \ref{eqn:p1_likelihood}: we'll
        have $\mathbb{P}\left(T = t\right)$ in place of
        $p\left(1 - p\right)^{t-1}$ and $\mathbb{P}\left(T > N\right)$ in place
        of $\left(1 - p\right)^N$.
      \end{description}
    \end{enumerate}
\end{enumerate}
